 ...
 */
    ...
    public:
        ...
        RC put(const key_t& key, const value_t& value) {
          ...
          if (it == _cache_items_map.end()) {   /* element not found */
        ...
        RC get(const key_t& key, value_t* res_value) {
          ...
          key_value_pair_t kv_pair = *(it->second);
          _cache_items_list.erase(it->second);
        ...
        RC victim(key_t old_key, key_t new_key) {
          ...
          value_t value = it->second->second;
          _cache_items_list.erase(it->second);
          _cache_items_list.push_front(std::make_pair(new_key, value));
          _cache_items_map.erase(old_key);
          _cache_items_map.insert(std::make_pair(new_key, _cache_items_list.begin()));
